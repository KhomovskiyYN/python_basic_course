...
file.close()
"""
Аналогичная парадигма - у вас есть чертеж машины - это класс с его полями и методами, и в этом чертеже есть конструктор - это метод (функция) которой если передать запчасти от машины он (конструктор) сможет сделать новый автомобиль. Экземпляр класса это то что сделалось по чертежу - это новый автомобиль
странно выглядит то что в определинии есть аргумент, а при вызове его нет
